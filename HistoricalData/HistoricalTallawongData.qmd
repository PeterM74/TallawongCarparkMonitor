...
## Calculate HPD
fCalculateHPD <- function(x, width = 0.9) {
  
  if (length(x) <= 2)
    return(list(list(LowerCI = median(x),UpperCI = median(x))))
  
  Output <- coda::HPDinterval(coda::mcmc(x), prob = width) %>%
    as.numeric()
...
Settings$EndDate <- lubridate::dmy("16012026")
...
Settings$SydenhamMetroOpen <- lubridate::dmy("19082024")
```
...
saveRDS(FinalHistoricalData, file = "TallawongData 2026-01-18.rds")
```
...
HistoricalReferenceDateDataPH <- tibble::tribble(
  ...
  "2025-01-01", "Public holiday",
  "2025-01-27", "Public holiday",
  "2025-04-18", "Public holiday",
  "2025-04-21", "Public holiday",
  "2025-04-25", "Public holiday",
  "2025-06-09", "Public holiday",
  "2025-10-06", "Public holiday",
  "2025-12-25", "Public holiday",
  "2025-12-26", "Public holiday",
  
  "2026-01-01", "Public holiday",
  "2026-01-26", "Public holiday",
  "2026-12-26", "Public holiday",
  "2026-04-03", "Public holiday",
  "2026-04-06", "Public holiday",
  "2026-04-25", "Public holiday",
  "2026-06-08", "Public holiday",
  "2026-10-05", "Public holiday",
...
HistoricalReferenceDateDataSH <- tibble::tibble(
  Date = c(
    ...
    seq.Date(from = lubridate::ymd("2025-04-14"),
             to = lubridate::ymd("2025-04-24"),
             by = "days"),
    ## Winter holidays
    seq.Date(from = lubridate::ymd("2025-07-07"),
             to = lubridate::ymd("2025-07-18"),
             by = "days"),
    ## Spring holidays
    seq.Date(from = lubridate::ymd("2025-09-29"),
             to = lubridate::ymd("2025-10-10"),
             by = "days"),
    ## Summer holidays (eastern)
    seq.Date(from = lubridate::ymd("2025-12-22"),
             to = lubridate::ymd("2026-01-26"),
             by = "days")
  ),
...
```{r LoadStoredData}
FinalHistoricalData <- readRDS(file = "TallawongData 2026-01-18.rds")
```
...
Add weights. As the area around Tallawong station is undergoing massive development, old data may not be as reliable as more recent data. The opening of the Sydenham extension increased patronage anecdotally so data will be limited to the Sydenham extension period. ~~Without spending a lot of time trying to quantify the change in development and impacts over time (as this project is meant to be done quickly for my personal benefit and not to be as accurate as possible), I have only applied a minor penalisation to historic data from this point (earliest data is worth 80% of the most up to date data) as it is still informative.~~ **Update: unweighted data will be used as the Sydenham metro opening was recent and there is insufficient data to warrant weighting out older data at the moment.**

```{r}
#| eval: false
StartDateInteger <- paste(Settings$SydenhamMetroOpen, "00:00:00") %>%
  as.character() %>%
  lubridate::ymd_hms() %>%
  as.integer()
...
```{r}
#| label: temporary unweighted section
StartDateSydenham <- paste(Settings$SydenhamMetroOpen, "00:00:00") %>%
  as.character() %>%
  lubridate::ymd_hms()


FinalHistoricalDataWeighted <- FinalHistoricalDataInterpolated %>%
  dplyr::filter(MinuteDate >= StartDateSydenham)
```

Create final summary dataframe for plotting, with the 90th Highest Percentile Density (HPD) for each datapoint.
...
ParkAvailabilityByDayTime <- FinalHistoricalDataWeighted %>%
  ...
  dplyr::summarise(N = dplyr::n(),
                   BootMedian = median(Available),
                   HPDResultsAvailable = fCalculateHPD(Available),
                   Min = min(Available, na.rm = TRUE),
                   Max = max(Available, na.rm = TRUE),
                   .groups = "drop") %>%
  suppressWarnings() %>%
  tidyr::unnest_wider(col = HPDResultsAvailable) %>%
